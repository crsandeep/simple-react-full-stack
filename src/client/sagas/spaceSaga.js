...
export function* handleGetSpaceList({userId}) {
    try {
        ...
        if(operResult.isSuccess){
            yield put(Actions.getSpaceList(operResult.data));
        }else{
            yield put(Actions.getSpaceList(null));
        }
    } catch (error) {
        yield put(Actions.getSpaceList(null));
    }
...
export function* handleGetSpace({spaceId}) {
    try {
        ...
        if(operResult.isSuccess){
            yield put(Actions.getSpace(operResult.data));
        }else{
            yield put(Actions.getSpace(null));
        }
    } catch (error) {
        yield put(Actions.getSpace(null));
    }
...
export function* handleRemoveSpaceImg({spaceId}) {
    try {
        ...
        if(operResult.isSuccess){
            yield put(Actions.completeRemoveSpaceImg(operResult.data));
        }else{
            yield put(Actions.failRemoveSpaceImg(operResult.message));
        }
    } catch (error) {
        yield put(Actions.failRemoveSpaceImg(error.message));
    }
}
